...
if exist('results_complete_ordered_tweeter','var')
   clearvars -except results_complete_ordered_tweeter
else
    clear all
end
...
results_complete_ordered_woofer = zeros(tot_subj, tot_combinations);
for k = 1:numel(files)
    load(names(k));
    
    % Convert to doubles the values!
    results_complete_ordered_woofer(k,:) = str2double(results);
end
